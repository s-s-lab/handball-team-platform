...
import {
  assignMatchSeasonAction,
  saveSeasonAction,
  saveSeasonPlayerStatsAction,
} from "@/features/season-stats/actions";
import {
  goalsPerAppearance,
  savePercentage,
...
export function SeasonStatsBoard({
  teamId,
  teamName,
  isAdmin,
  seasons,
    ...
    <div className="space-y-8 pb-10">
            ...
            <nav aria-label="シーズン選択" className="flex flex-wrap gap-2">
              {seasons.map((season) => (
                <Button key={season.id} asChild size="sm" variant={season.id === selectedSeason?.id ? "default" : "outline"}>
                  <Link href={`/app/teams/${teamId}/stats?season=${season.id}`}>
                    {season.name}{season.isCurrent ? " · 現在" : ""}
                  </Link>
                </Button>
              ))}
            </nav>
      ...
      {!selectedSeason ? (
        <section className="border border-dashed border-border bg-muted/30 p-8 text-center">
          ...
          {isAdmin ? <SeasonForm teamId={teamId} compact /> : null}
        </section>
      ) : (
        <>
          ...
          <section>
            ...
            <form action={isAdmin ? saveSeasonPlayerStatsAction : undefined} className="mt-4">
                    ...
                    {players.map((player) => (
                      <tr key={player.teamMemberId} className="bg-card align-middle">
                        <td className="px-4 py-3">
                          {isAdmin ? <input type="hidden" name="memberId" value={player.teamMemberId} /> : null}
                          <div className="font-black">#{player.shirtNumber ?? "–"} {player.displayName}</div>
                          <div className="text-xs text-muted-foreground">{player.primaryPosition ?? "–"}</div>
                        </td>
                        {isAdmin ? (
                          <>
                            ...
                            <td className="pr-4"><input name={`notes:${player.teamMemberId}`} defaultValue={player.notes ?? ""} maxLength={2000} className="h-10 w-52 border border-border bg-background px-2 outline-none focus:border-[var(--workspace-accent)]" /></td>
                          </>
      ...
      {isAdmin ? (
        <div className="grid gap-8 xl:grid-cols-2">
          <section className="border-t-4 border-[var(--workspace-accent)] bg-card p-5 shadow-sm">
            <h2 className="text-xl font-black">{selectedSeason ? "シーズン設定" : "新しいシーズン"}</h2>
            <p className="mt-1 text-sm text-muted-foreground">名称・期間・現在シーズンを管理します。</p>
            {selectedSeason ? <SeasonForm teamId={teamId} season={selectedSeason} /> : <SeasonForm teamId={teamId} />}
            {selectedSeason ? (
              <details className="mt-6 border-t border-border pt-4">
                <summary className="cursor-pointer text-sm font-black text-muted-foreground">新しいシーズンを追加</summary>
          ...
          <section className="border-t-4 border-[var(--workspace-highlight)] bg-card p-5 shadow-sm">
            ...
            <div className="mt-4 space-y-3">
              {matches.map((match) => (
                <form action={assignMatchSeasonAction} key={match.id} className="grid gap-2 border-b border-border pb-3 sm:grid-cols-[1fr_12rem_auto] sm:items-center">
                  <input type="hidden" name="teamId" value={teamId} />
...
function SeasonForm({ teamId, season, compact = false }: { teamId: string; season?: SeasonSummary; compact?: boolean }) {
  return (
    <form action={saveSeasonAction} className={`mt-4 grid gap-3 ${compact ? "mx-auto max-w-lg text-left" : ""}`}>
      <input type="hidden" name="teamId" value={teamId} />
      {season ? <input type="hidden" name="seasonId" value={season.id} /> : null}
      <label className="text-sm font-bold">シーズン名<input name="name" required maxLength={80} defaultValue={season?.name ?? ""} placeholder="2026-27" className="mt-1 h-11 w-full border border-border bg-background px-3 font-semibold outline-none focus:border-[var(--workspace-accent)]" /></label>
      <div className="grid gap-3 sm:grid-cols-2">
        <label className="text-sm font-bold">開始日<input name="startDate" type="date" required defaultValue={season?.startDate ?? ""} className="mt-1 h-11 w-full border border-border bg-background px-3" /></label>
        <label className="text-sm font-bold">終了日<input name="endDate" type="date" required defaultValue={season?.endDate ?? ""} className="mt-1 h-11 w-full border border-border bg-background px-3" /></label>
      </div>
      <label className="flex min-h-11 items-center gap-2 text-sm font-bold"><input name="isCurrent" type="checkbox" value="true" defaultChecked={season?.isCurrent ?? false} /> 現在のシーズンにする</label>
      <Button type="submit" className="justify-self-start">{season ? "シーズン情報を更新" : "シーズンを作成"}</Button>
    </form>
  );
}
